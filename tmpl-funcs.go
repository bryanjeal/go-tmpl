...
// Modifications to this file are minimal. Functions were removed/rewritten
// only to be able to use the various template functions outside of Hugo
...
import (
	...
	hugoHelpers "github.com/spf13/hugo/helpers"
)
...
func compareGetFloat(a interface{}, b interface{}) (float64, float64) {
	...
	case reflect.Struct:
		switch av.Type() {
		case helpers.Reflect.TimeType:
			left = float64(helpers.Reflect.ToTimeUnix(av))
		}
	}
	...
	case reflect.Struct:
		switch bv.Type() {
		case helpers.Reflect.TimeType:
			right = float64(helpers.Reflect.ToTimeUnix(bv))
		}
	}
...
func in(l interface{}, v interface{}) bool {
	...
	case reflect.Array, reflect.Slice:
		for i := 0; i < lv.Len(); i++ {
			lvv := lv.Index(i)
			lvv, isNil := helpers.Reflect.Indirect(lvv)
			if isNil {
				continue
			}
			switch lvv.Kind() {
...
func first(limit interface{}, seq interface{}) (interface{}, error) {
	...
	seqv, isNil := helpers.Reflect.Indirect(seqv)
	if isNil {
...
func evaluateSubElem(obj reflect.Value, elemName string) (reflect.Value, error) {
	if !obj.IsValid() {
		return helpers.Reflect.Zero, errors.New("can't evaluate an invalid value")
	}
	typ := obj.Type()
	obj, isNil := helpers.Reflect.Indirect(obj)
	...
	if ok {
		if mt.PkgPath != "" {
			return helpers.Reflect.Zero, fmt.Errorf("%s is an unexported method of type %s", elemName, typ)
		}
		...
		if mt.Type.NumIn() > 1 || mt.Type.NumOut() == 0 || mt.Type.NumOut() > 2 {
			return helpers.Reflect.Zero, fmt.Errorf("%s is a method of type %s but doesn't satisfy requirements", elemName, typ)
		}
		if mt.Type.NumOut() == 1 && mt.Type.Out(0).Implements(helpers.Reflect.ErrorType) {
			return helpers.Reflect.Zero, fmt.Errorf("%s is a method of type %s but doesn't satisfy requirements", elemName, typ)
		}
		if mt.Type.NumOut() == 2 && !mt.Type.Out(1).Implements(helpers.Reflect.ErrorType) {
			return helpers.Reflect.Zero, fmt.Errorf("%s is a method of type %s but doesn't satisfy requirements", elemName, typ)
		}
		res := objPtr.Method(mt.Index).Call([]reflect.Value{})
		if len(res) == 2 && !res[1].IsNil() {
			return helpers.Reflect.Zero, fmt.Errorf("error at calling a method %s of type %s: %s", elemName, typ, res[1].Interface().(error))
		}
		return res[0], nil
	}
	...
	// a nil value
	if isNil {
		return helpers.Reflect.Zero, fmt.Errorf("can't evaluate a nil pointer of type %s by a struct field or map key name %s", typ, elemName)
	}
	switch obj.Kind() {
	case reflect.Struct:
		...
		if ok {
			if ft.PkgPath != "" && !ft.Anonymous {
				return helpers.Reflect.Zero, fmt.Errorf("%s is an unexported field of struct type %s", elemName, typ)
			}
			return obj.FieldByIndex(ft.Index), nil
		}
		return helpers.Reflect.Zero, fmt.Errorf("%s isn't a field of struct type %s", elemName, typ)
	case reflect.Map:
		kv := reflect.ValueOf(elemName)
		if kv.Type().AssignableTo(obj.Type().Key()) {
			return obj.MapIndex(kv), nil
		}
		return helpers.Reflect.Zero, fmt.Errorf("%s isn't a key of map type %s", elemName, typ)
	}
	return helpers.Reflect.Zero, fmt.Errorf("%s is neither a struct field, a method nor a map element of type %s", elemName, typ)
}

func checkCondition(v, mv reflect.Value, op string) (bool, error) {
	v, vIsNil := helpers.Reflect.Indirect(v)
	if !v.IsValid() {
		vIsNil = true
	}
	mv, mvIsNil := helpers.Reflect.Indirect(mv)
	if !mv.IsValid() {
		mvIsNil = true
	}
	if vIsNil || mvIsNil {
	...
	if mv.Type() == v.Type() {
		...
		case reflect.Struct:
			switch v.Type() {
			case helpers.Reflect.TimeType:
				iv := helpers.Reflect.ToTimeUnix(v)
				ivp = &iv
				imv := helpers.Reflect.ToTimeUnix(mv)
				imvp = &imv
			}
		case reflect.Array, reflect.Slice:
			slv = v.Interface()
		...
		case reflect.Int, reflect.Int8, reflect.Int16, reflect.Int32, reflect.Int64:
			...
			ivp = &iv
			for i := 0; i < mv.Len(); i++ {
				if anInt := helpers.Reflect.ToInt(mv.Index(i)); anInt != -1 {
		...
		case reflect.String:
			...
			svp = &sv
			for i := 0; i < mv.Len(); i++ {
				if aString := helpers.Reflect.ToString(mv.Index(i)); aString != "" {
		...
		case reflect.Struct:
			...
			case helpers.Reflect.TimeType:
				...
				ivp = &iv
				for i := 0; i < mv.Len(); i++ {
					ima = append(ima, helpers.Reflect.ToTimeUnix(mv.Index(i)))
				}
			}
		}
	}
...
func checkWhereArray(seqv, kv, mv reflect.Value, path []string, op string) (interface{}, error) {
	...
	for i := 0; i < seqv.Len(); i++ {
		...
		if kv.Kind() == reflect.String {
			...
			vv, _ := helpers.Reflect.Indirect(rvv)
...
func checkWhereMap(seqv, kv, mv reflect.Value, path []string, op string) (interface{}, error) {
	...
	for _, k := range keys {
		...
		case reflect.Interface:
			elemvv, isNil := helpers.Reflect.Indirect(elemv)
			if isNil {
				continue
			}
...
func where(seq, key interface{}, args ...interface{}) (interface{}, error) {
	seqv, isNil := helpers.Reflect.Indirect(reflect.ValueOf(seq))
	if isNil {
...
func returnWhenSet(a, k interface{}) interface{} {
	av, isNil := helpers.Reflect.Indirect(reflect.ValueOf(a))
	if isNil {
		return ""
	}
	...
	avv, isNil = helpers.Reflect.Indirect(avv)
...
func highlight(in interface{}, lang, opts string) (template.HTML, error) {
	...
	return template.HTML(hugoHelpers.Highlight(html.UnescapeString(str), lang, opts)), nil
}
...
func emojify(in interface{}) (template.HTML, error) {
	...
	return template.HTML(hugoHelpers.Emojify([]byte(str))), nil
}
...
func plainify(in interface{}) (string, error) {
	...
	return hugoHelpers.StripHTML(s), nil
}
...
func index(item interface{}, indices ...interface{}) (interface{}, error) {
	...
	for _, i := range indices {
		...
		var isNil bool
		if v, isNil = helpers.Reflect.Indirect(v); isNil {
...
func countWords(content interface{}) (int, error) {
	...
	counter := 0
	for _, word := range strings.Fields(hugoHelpers.StripHTML(conv)) {
...
func countRunes(content interface{}) (int, error) {
	...
	counter := 0
	for _, r := range hugoHelpers.StripHTML(conv) {
		if !hugoHelpers.IsWhitespace(r) {
			counter++
		}
	}
...
// Partial is the handler for "partial" template function (FuncMap)
// It will add the template to the store if needed and execute it
func (t *TplSys) Partial(name string, ctxs ...interface{}) template.HTML {
	// trim "path" from partial template name
	name = strings.TrimPrefix(name, t.BaseDir()+"partials/")
	name = strings.TrimPrefix(name, "/")
	name = strings.TrimPrefix(name, "partials/")

	// context to pass along to template renderer
	var ctx interface{}

	// if no context is provided the pass nil
	// otherwise only use the first one
	if len(ctxs) == 0 {
		ctx = nil
	} else {
		ctx = ctxs[0]
	...
	// make sure partial template is in the store
	// if it isn't then add it
	_, err := t.getTemplate(name)
	if err == ErrTmplNotFound {
		_, err := t.AddTemplate(name, "", "", t.BaseDir()+"partials/"+name)
		if err != nil {
	...
	// execute template
	b, err := t.ExecuteTemplate(name, ctx)
	if err != nil {
		log.Println(err.Error())
		return template.HTML("")
	}
...
func (t *TplSys) genFuncMap() template.FuncMap {
	funcMap = template.FuncMap{
		"add":          func(a, b interface{}) (interface{}, error) { return hugoHelpers.DoArithmetic(a, b, '+') },
		"after":        after,
		"apply":        apply,
		...
		"delimit":      delimit,
		"dict":         dictionary,
		"div":          func(a, b interface{}) (interface{}, error) { return hugoHelpers.DoArithmetic(a, b, '/') },
		...
		"mod":          mod,
		"modBool":      modBool,
		"mul":          func(a, b interface{}) (interface{}, error) { return hugoHelpers.DoArithmetic(a, b, '*') },
		"ne":           ne,
		"partial":      t.Partial,
		"plainify":     plainify,
		"pluralize":    pluralize,
		...
		"safeJS":       safeJS,
		"safeURL":      safeURL,
		"sanitizeURL":  hugoHelpers.SanitizeURL,
		"sanitizeurl":  hugoHelpers.SanitizeURL,
		"seq":          hugoHelpers.Seq,
		"sha1":         sha1,
		"shuffle":      shuffle,
		...
		"sub":          func(a, b interface{}) (interface{}, error) { return hugoHelpers.DoArithmetic(a, b, '-') },
		"substr":       substr,
